...
export default function ComputerUsePage({ state, notify }: PageProps) {
  ...
  if (!desktop) return <>
    <PageHead title="电脑操作" description="该功能需要 Windows 桌面客户端的受控本机桥接。浏览器版不会读取窗口、截图或执行输入。" />
    <section className="cu-browser panel"><Empty title="请在 Windows 客户端中打开" detail="网页版本可继续使用虚构聊天环境，验证自动回复与人工复制流程。" action={<Button onClick={() => navigate('autopilot')}>打开回复模式实验室<ExternalLink size={16} /></Button>} /></section>
  </>;
  ...
  return <>
    <PageHead eyebrow="WINDOWS COMPUTER USE" title="电脑操作" description="选择一个窗口，再由模型根据窗口画面逐步操作。自动模式可输入并发送；人工模式只生成操作草稿。" />
    <div className="cu-layout">
      <div className="cu-main">
        <section className="panel cu-panel"><div className="cu-section-head"><span><KeyRound size={18} /></span><div><h2>{gemini ? 'Gemini 原生 Computer Use' : '视觉动作模型'}</h2><p>{gemini ? '使用 Gemini 原生 Computer Use 接口与 API Key；不是 Antigravity OAuth，也不使用 OpenAI 兼容接口。' : '需要支持 UI-TARS 动作协议的视觉模型，不是普通聊天模型。'}</p></div></div>
          <div className="form-row"><Field label="模型协议"><select aria-label="电脑操作模型协议" disabled={locked} value={settings.modelFamily} onChange={e => { const modelFamily = e.target.value as ComputerUseSettings['modelFamily']; setSettings(modelFamily === 'gemini' ? { ...settings, modelFamily, baseUrl: geminiRootBaseUrl, model: geminiRecommendedModel } : { ...settings, modelFamily }); }}><option value="gemini">Gemini 原生 Computer Use</option><option value="ui-tars">UI-TARS</option><option value="doubao">豆包 Computer Use</option></select></Field>{gemini ? <Field label="原生接口地址"><input aria-label="Gemini 原生接口地址" value={geminiRootBaseUrl} disabled /></Field> : <Field label="接口地址"><input aria-label="电脑操作接口地址" disabled={locked} value={settings.baseUrl} placeholder="https://…/v1" onChange={e => setSettings({ ...settings, baseUrl: e.target.value })} /></Field>}</div>
        ...
        <section className="panel cu-panel"><div className="cu-section-head"><span><Monitor size={18} /></span><div><h2>明确选择目标窗口</h2><p>不会自动枚举或截图。请主动打开内置测试窗口，或手动列出本机 Windows 窗口。</p></div></div>
          <div className="cu-actions"><Button variant="secondary" busy={busy} disabled={locked} onClick={() => void openFixture()}>打开内置测试窗口</Button><Button variant="secondary" busy={busy} disabled={locked} onClick={() => void listTargets()}>列出 Windows 窗口</Button></div>
          {targets.length > 0 && <div className="cu-targets" role="radiogroup" aria-label="电脑操作目标窗口">{targets.map(target => <label key={target.id} className={selectedTarget === target.id ? 'selected' : ''}><input disabled={locked} type="radio" name="computer-target" checked={selectedTarget === target.id} onChange={() => setSelectedTarget(target.id)} /><Target size={15} /><span>{target.name}</span><small>{target.kind === 'test' ? '内置测试' : 'Windows 窗口'}</small></label>)}</div>}
        </section>
        <section className="panel cu-panel"><div className="cu-section-head"><span><WandSparkles size={18} /></span><div><h2>本次操作</h2><p>知识条目默认为不选择。模型只会收到你勾选的内容与所选窗口截图。</p></div></div>
          ...
          <div className="form-row"><Field label="执行方式"><select aria-label="电脑操作执行方式" disabled={locked} value={mode} onChange={e => setMode(e.target.value as ComputerUseMode)}><option value="manual">人工复制回复</option><option value="auto">自动回复</option></select></Field><Field label="必需硬件"><input aria-label="电脑操作必需硬件" value="Pico USB HID（已固定）" disabled /></Field><Field label="最大步骤"><input aria-label="最大操作步骤" disabled={locked} type="number" min="1" max="25" value={maxSteps} onChange={e => setMaxSteps(Math.max(1, Math.min(25, Number(e.target.value) || 1)))} /></Field></div><div className="notice">电脑操作的自动与人工草稿模式都必须先连接匹配固件的 Pico 1；USB 断开或异常会停止任务，绝不回退 Windows 软件输入。{gemini ? '逐键输入使用 Gemini 原生 Computer Use 的模型 Key 作候选识别，不另设普通视觉模型。' : '逐键输入使用“设置”中的普通视觉模型识别候选词。'}请最大化目标窗口，让候选词完整可见，并从空白输入框开始；输入中断后先人工清理未提交拼音和草稿。{usbStatus?.connected ? (usbStatus.armed ? ' 软件任务会话活动中。' : ' 已连接，等待软件开始。') : ' 当前未连接。'}</div>
          ...
          <div className="cu-start"><div><ShieldCheck size={17} /><p>{awaitingConfirmation ? '模型提出了下一步操作，等待你确认或拒绝；当前仍可停止任务。' : settingsDirty ? '模型设置有未保存修改，请先保存后再开始。' : !savedSettings.hasKey ? `请先保存 ${gemini ? 'Gemini 原生接口的' : '支持 UI-TARS 动作协议的模型'} API Key。` : !usbStatus?.connected ? '请先连接匹配固件的 Pico USB HID；离线时只能查看管理与烧录说明。' : mode === 'auto' ? '自动模式会向模型发送所选窗口截图与知识，并可能在目标窗口输入、点击或发送。' : '人工模式也需要 Pico USB HID 已连接；只生成草稿，不会自动发送。'}</p></div><Button disabled={!selected || !instruction.trim() || busy || locked || settingsDirty || !savedSettings.hasKey || !usbStatus?.connected} onClick={() => setConfirmStart(true)}><Play size={16} />开始前确认</Button></div>
        </section>
      </div>
    ...
    {confirmStart && selected && <Modal title="确认开始电脑操作" subtitle="这一步才会把模型请求发送到你配置的接口，并使用已连接的 Pico USB HID。" onClose={() => setConfirmStart(false)}><div className="cu-confirm"><p><strong>目标窗口：</strong>{selected.name}</p><p><strong>已保存模型：</strong>{savedSettings.model || '未设置'} · {savedSettings.baseUrl || '未设置接口地址'}</p><p><strong>执行方式：</strong>{mode === 'auto' ? '自动回复：模型可在目标窗口输入、点击或发送。' : '人工复制回复：只生成草稿，不会自动发送。'}</p><p><strong>硬件：</strong>Pico USB HID 已连接；断开或异常会停止任务，不回退 Windows 软件执行器。</p><p><strong>发送给模型：</strong>所选窗口截图（含输入法候选词）、操作指令，以及 {knowledgeIds.length} 条所选业务知识。</p></div><div className="modal-actions"><Button variant="secondary" onClick={() => setConfirmStart(false)}>返回检查</Button><Button busy={busy} disabled={!usbStatus?.connected} onClick={() => void start()}><Upload size={16} />确认并开始</Button></div></Modal>}
  </>;
}
