...
import { Bot, Check, Clipboard, Download, FileText, Monitor, Pause, Play, Plus, RefreshCw, Settings, Sparkles, Square, Target, Trash2, Users } from 'lucide-react';
...
export default function DesktopRepliesPage({ state, notify }: PageProps) {
  ...
  const setupDeepSeek = async () => {
    ...
    try {
      ...
      setConfig(current => ({ ...current, modelProtocol: 'openai-vision' }));
      notify('已载入 DeepSeek 官方配置 (deepseek-chat)！请在“设置”页保存你的 DeepSeek API Key。');
      await refresh();
    } catch (error) {
  ...
  return <>
    ...
    <div className="panel dr-private-domain-bar" style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '12px 18px', marginBottom: '16px', borderRadius: '8px', background: 'rgba(255,255,255,0.02)', border: '1px solid rgba(255,255,255,0.08)' }}>
      ...
      <div style={{ display: 'flex', gap: '8px' }}>
        ...
        <Button variant="secondary" busy={busy} onClick={() => void setupDeepSeek()}><Bot size={15} />一键 DeepSeek</Button>
      </div>
    </div>
    ...
    </div><aside className="panel dr-status"><div className="panel-head"><div><h2>持续运行状态</h2><p>{replyState?.message || '尚未加载运行状态。'}</p></div><span className={`dr-status-pill ${replyState?.status || 'stopped'}`}>{replyState?.status === 'running' ? '运行中' : replyState?.status === 'paused' ? '已暂停' : replyState?.status === 'needs_attention' ? '需要处理' : '已停止'}</span></div><div className="dr-metrics"><span>扫描轮次<strong>{replyState?.cycle || 0}</strong></span><span>下一次<strong>{replyState?.nextScanAt ? new Date(replyState.nextScanAt).toLocaleTimeString('zh-CN', { hour: '2-digit', minute: '2-digit' }) : '—'}</strong></span></div><div className="dr-run-actions">{running ? <Button variant="secondary" disabled={busy} onClick={() => void desktop.pause().then(setReplyState).catch(error => notify((error as Error).message, true))}><Pause size={15} />暂停</Button> : <Button disabled={busy || dirty || !selected || !provider?.configured || config.conversations.length === 0 || !usbStatus?.connected} onClick={() => setConfirmStart(true)}><Play size={15} />启动前确认</Button>}<Button variant="danger" disabled={!busy && replyState?.status === 'stopped'} onClick={() => void desktop.stop().then(value => { setReplyState(value); notify('持续回复已停止'); }).catch(error => notify((error as Error).message, true))}><Square size={15} />停止</Button></div>{dirty && <p className="dr-warning">配置有未保存修改；请保存后再启动。</p>}<div className="dr-jobs"><h3>回复任务</h3>{replyState?.jobs.length ? replyState.jobs.slice(0, 30).map(job => <article key={job.id}><div><strong>{job.conversationName}</strong><p>{job.input}</p>{job.psychology && <div style={{ margin: '8px 0', padding: '8px 10px', borderRadius: '6px', background: 'rgba(56, 189, 248, 0.05)', border: '1px solid rgba(56, 189, 248, 0.18)', fontSize: '11px' }}><div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '4px' }}><div style={{ display: 'flex', gap: '6px', alignItems: 'center' }}><span style={{ fontWeight: 600, color: '#38bdf8' }}>🧠 心理诊断</span><span style={{ padding: '1px 5px', borderRadius: '3px', fontSize: '10px', fontWeight: 600, background: job.psychology.riskLevel === 'high_risk' ? 'rgba(239, 68, 68, 0.2)' : job.psychology.riskLevel === 'cautious' ? 'rgba(245, 158, 11, 0.2)' : 'rgba(34, 197, 94, 0.2)', color: job.psychology.riskLevel === 'high_risk' ? '#f87171' : job.psychology.riskLevel === 'cautious' ? '#fbbf24' : '#4ade80' }}>{job.psychology.riskLevel === 'high_risk' ? '🔴 高危客诉' : job.psychology.riskLevel === 'cautious' ? '🟡 谨慎观望' : '🟢 安全沟通'}</span><span style={{ padding: '1px 5px', borderRadius: '3px', fontSize: '10px', background: 'rgba(255,255,255,0.06)', color: '#cbd5e1' }}>{job.psychology.coreNeed === 'price' ? '💰 价格敏感' : job.psychology.coreNeed === 'trust' ? '🛡️ 信任求证' : job.psychology.coreNeed === 'speed' ? '⚡ 交付时效' : job.psychology.coreNeed === 'service' ? '🔧 售后支持' : job.psychology.coreNeed === 'reassurance' ? '🤝 降低试错' : '💬 基础咨询'}</span></div><span style={{ color: '#94a3b8', fontSize: '10px' }}>把握度 {job.psychology.confidence}%</span></div><p style={{ margin: '2px 0', color: '#e2e8f0', lineHeight: 1.4 }}><strong>真实意图：</strong>{job.psychology.underlyingIntent}</p><p style={{ margin: '2px 0', color: '#38bdf8', lineHeight: 1.4 }}><strong>战术建议：</strong>{job.psychology.suggestedAction}</p></div>}{job.candidates && job.candidates.length > 0 && <div style={{ margin: '6px 0', display: 'flex', gap: '6px', flexWrap: 'wrap' }}>{job.candidates.map(c => <button key={c.id} type="button" onClick={() => void desktop?.selectCandidate(job.id, c.id).then(setReplyState)} style={{ padding: '3px 8px', fontSize: '11px', borderRadius: '4px', cursor: 'pointer', background: (job.selectedCandidateId ?? 'warm') === c.id ? 'rgba(56, 189, 248, 0.25)' : 'rgba(255,255,255,0.04)', border: (job.selectedCandidateId ?? 'warm') === c.id ? '1px solid #38bdf8' : '1px solid rgba(255,255,255,0.1)', color: (job.selectedCandidateId ?? 'warm') === c.id ? '#38bdf8' : '#94a3b8', fontWeight: (job.selectedCandidateId ?? 'warm') === c.id ? 600 : 400 }} title={c.rationale}>{c.label}</button>)}</div>}{job.reply && <p className="dr-reply">{job.reply}</p>}<small>{statusNames[job.status]} · {job.detail || '等待更新'}</small>{job.status === 'uncertain' && <p className="dr-warning">请先检查发送记录，并人工清理未提交拼音或残留草稿，再恢复会话；此任务不会自动重试。</p>}</div><div>{job.reply && job.status !== 'sending' && <Button variant="secondary" onClick={() => void desktop.copy(job.id).then(() => notify('回复已复制')).catch(error => notify((error as Error).message, true))}><Clipboard size={14} />复制</Button>}{job.status === 'uncertain' && <Button variant="secondary" onClick={() => void desktop.resolve(job.id).then(setReplyState).catch(error => notify((error as Error).message, true))}>标记已处理</Button>}</div></article>) : <p className="muted">启动后会在这里显示生成、待复制、视觉确认或结果不明的任务。</p>}</div><div className="dr-takeovers"><h3>会话人工接管</h3>{config.conversations.map(item => { const active = replyState?.config.conversations.find(row => row.id === item.id); const takingOver = active ? !active.enabled : !item.enabled; return <label key={item.id}><span>{item.name}</span><input type="checkbox" checked={takingOver} onChange={() => void changeTakeover(item.id, takingOver)} disabled={busy} />人工接管</label>; })}</div></aside></div>
