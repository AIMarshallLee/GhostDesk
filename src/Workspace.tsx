...
function KnowledgeEditor({ item, busy, onClose, onSave }: { item?: Knowledge; busy: boolean; onClose: () => void; onSave: (data: unknown) => Promise<void> }) {
  const [title, setTitle] = useState(item?.title || ''); const [content, setContent] = useState(item?.content || ''); const [tags, setTags] = useState(item?.tags.join('，') || '');
  return <Modal title={item ? '编辑业务知识' : '添加业务知识'} subtitle="写清事实与适用条件，助手才能引用得准确。" onClose={onClose}><form onSubmit={e => { e.preventDefault(); void onSave({ title, content, tags: [...new Set(tags.split(/[,，]/).map(t => t.trim()).filter(Boolean))], enabled: item?.enabled ?? true }); }}><Field label="知识标题"><input required maxLength={200} placeholder="例如：标准版交付与售后说明" value={title} onChange={e => setTitle(e.target.value)} /></Field><Field label="知识内容"><textarea required rows={9} maxLength={20000} placeholder="输入准确的业务资料。建议包含适用范围、具体规则、例外情况。" value={content} onChange={e => setContent(e.target.value)} /></Field><Field label="标签" hint="用逗号分隔，例如：售后，产品说明"><input value={tags} maxLength={300} onChange={e => setTags(e.target.value)} placeholder="产品说明，常见问题" /></Field><div className="modal-actions"><Button type="button" variant="secondary" onClick={onClose}>取消</Button><Button type="submit" busy={busy}><Save size={16} />保存知识</Button></div></form></Modal>;
}
